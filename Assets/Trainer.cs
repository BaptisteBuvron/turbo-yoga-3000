...
public class Trainer : MonoBehaviour
{
  ...
  public void TransitionToAnimation(string nomAnimation)
  {
    if (!animationMethodAlreadyCalled)
    {
      ...
      {
        Debug.Log("Reversing animation " + lastAnimation);
        StartAnimationAndWait(lastAnimation, true);
      }
      Debug.Log("Playing animation " + nomAnimation);
      StartAnimationAndWait(nomAnimation, false);
  ...
  IEnumerator StartAnimationAndWait(string animationName, bool reverse = false)
  {
    animator.Play(animationName);
    animator.SetFloat("forward", reverse ? -1 : 1);
